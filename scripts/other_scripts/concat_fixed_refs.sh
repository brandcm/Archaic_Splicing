...
#$ -m ae
#$ -cwd
#$ -o ~/../../../group/capra/projects/archaic_splicing/scripts/other_scripts/concat_fixed_refs.out
#$ -e ~/../../../group/capra/projects/archaic_splicing/scripts/other_scripts/concat_fixed_refs.err
#$ -l h_rt=6:00:00
#$ -l mem_free=20G
...
if [ ! -f fixed_ref_annotations.txt ]; then
    tail -n +29 fixed_refs_*_annotated.vcf | grep '^[^=]' >> fixed_ref_annotations.txt
fi
...
awk '{print $1,$2,$4,$10,$11,$12,$13,$8}' OFS='\t' fixed_ref_annotations.txt | awk '{sub(/.*SpliceAI=/, "", $8)}1' | tr '|' '\t' | tr ',' '\n' |
awk 'NF==10{print p"\t"q"\t"r"\t"s"\t"t"\t"u"\t"v"\t"$1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9"\t"$10; next} {p=$1}{q=$2}{r=$3}{s=$4}{t=$5}{u=$6}{v=$7} 1' |
awk 'NF==17{print}{}' | tr ' ' '\t' > ../spliceai_annotated_fixed_refs.txt
